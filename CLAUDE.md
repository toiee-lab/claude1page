...
- Tailwind CSS v4.1 以上は、**Tailwind CLI でビルドされたCSSファイルを使用してください**
  - HTMLの `<head>` 内で、以下のように読み込んでください: `<link href="/assets/css/style.css" rel="stylesheet">`
  - CSS のビルドは、HTML ファイルを作成・編集すると**自動的に実行されます**（手動実行は不要）
  - カラーパレットのカスタマイズが必要な場合は、`src/input.css` の `@theme` セクションを編集してください
...
- カラーパレットは `src/input.css` の `@theme` セクションで設定（ユーザーの要求や、ユーザーの伝えたいコンテンツに最適なものを選び設定する）
- フォントは日本語に最適なものを選択、パフォーマンスを考慮する
- ホワイトスペースを効果的に使い、読みやすさを重視

### Tailwind CSS のテーマカスタマイズ

- ユーザーからカラーパレットの指定がある場合、または、コンテンツに最適なカラーパレットを選択する場合は、`src/input.css` の `@theme` セクションを編集してください
- カラー定義は、OKLCH形式を使用してください（例: `--color-primary-500: oklch(62% 0.15 250);`）
- 以下のような構造でカラーパレットを定義してください:
  ```css
  @theme {
    --color-primary-50: oklch(98.3% 0.02 250);
    --color-primary-100: oklch(95.1% 0.03 250);
    --color-primary-500: oklch(62% 0.15 250);
    --color-primary-900: oklch(22% 0.05 250);
  }
  ```
- テーマファイルを編集すると、自動的にCSSが再ビルドされます
